...
SpringConstraint::SpringConstraint(float elasticity, Body* bodyA, Body* bodyB) :
	...
	restingDistance((bodyB->getPosition() - bodyA->getPosition()).length())
{
}
...
void SpringConstraint::apply()
{
	...
	float displacementMagnitude = displacement.length() - restingDistance;
	...
	if (debugModel != NULL)
	{
		...
		debugModel->setColor(Vector3(1.0f - stretchedness, 0.0f, stretchedness));
