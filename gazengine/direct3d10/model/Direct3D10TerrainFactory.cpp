...
namespace Direct3D10TerrainFactory
{
	...
	{
		vector<D3DXVECTOR3> points;
		points.push_back(D3DXVECTOR3(width / 2.0f * -1.0f, 0.0f, 0.0f));
		points.push_back(D3DXVECTOR3(width / 2.0f, 0.0f, 0.0f));

		for (unsigned int iteration = 0; iteration < iterations; iteration++)
		{
			vector<D3DXVECTOR3> subdividedPoints;
			float randomRange = width / (2.0f * pow((float) iterations, 1.75f));

			for (unsigned int segmentIndex = 0; segmentIndex < points.size() - 1; segmentIndex++)
			{
				D3DXVECTOR3 difference = points.at(segmentIndex + 1) - points.at(segmentIndex);
				D3DXVECTOR3 halfDifference = difference / 2.0f;
				D3DXVECTOR3 midpoint = points.at(segmentIndex) + halfDifference;
