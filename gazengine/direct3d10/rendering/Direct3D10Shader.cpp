...
void Direct3D10Shader::setVar(const string& structName, const string& name, D3DXCOLOR value)
{
	effect->GetVariableByName(structName.data())->GetMemberByName(name.data())->AsVector()->SetFloatVector(value);
}

void Direct3D10Shader::setVar(const string& structName, const string& name, D3DXMATRIX value)
{
	effect->GetVariableByName(structName.data())->GetMemberByName(name.data())->AsMatrix()->SetMatrix(value);
}

void Direct3D10Shader::setVar(const string& structName, const string& name, D3DXVECTOR3 value)
{
	effect->GetVariableByName(structName.data())->GetMemberByName(name.data())->AsVector()->SetFloatVector(value);
}
